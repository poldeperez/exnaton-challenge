...
export class MeterController {
    ...
    @ApiOperation({ 
        ...
        description: 'Fetches meter data from a public URL and stores it in the database'
    })
    @ApiResponse({ status: 201, description: 'Data successfully ingested' })
    @ApiResponse({ status: 400, description: 'Bad request - invalid URL or data format' })
    ...
    async getMeasurements(
        ...
        if (!solarMeterId || !solarObisCode || !consumptionMeterId || !consumptionObisCode || !installationId || !measurement || !start || !stop) {
        ...
        if (!['d', 'w', 'm', 'y'].includes(interval)) {
            throw new BadRequestException('Invalid interval, must be one of d, w, m, y');
        }
    ...
    async testCache() {
        try {
            await this.meterService['cacheManager'].set('test-key', { hello: 'world' }, 60);
